...
Prefer consolidated LLM-friendly docs before scanning large documentation trees:

- Check for `llms.txt` or `llms-full.txt` in `docs/` or the repo root.
- If absent and documentation context is needed, run the project's documented docs-build command, then re-check the generated files.
...
Default to normal shell, local files, and `gh`.
Prefer read-only `gh` for inspection (pr/run view, diff, checks, list). Never run mutating `gh` unless explicitly asked.
...
- Never read, dump, or print secrets through any path, including bash tools (`bat`, `rg`, `cat`) that can bypass the read-tool denylist. This covers `.env*`, private and SSH keys, tokens, credentials, password stores, kubeconfigs, cloud-provider secrets, and production certificates.
...
- Prefer targeted search and symbol/LSP lookups (serena) over reading or dumping whole files.
- Read only relevant sections.
- Avoid huge pasted diffs unless needed.
- Use `opencode stats` outside the TUI to inspect usage and cost.
